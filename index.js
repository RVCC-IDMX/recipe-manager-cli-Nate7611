...
yargs(hideBin(process.argv))
  ...
  .command('create', 'Create a new recipe', () => {
    ...
    promptForRecipeInfo().then(function (recipeInfo) {
      ...
      console.log(`Recipe "${recipeInfo.name}" created successfully!`);
    }).catch(error => {
  ...
  .command('add-ingredient <id>', 'Add ingredient to a recipe', (yargs) => {
    ...
    getRecipeById(argv.id, function (recipe) {
      ...
      promptForIngredient().then(function (ingredientInfo) {
        ...
        const { name, amount, unit } = ingredientInfo; // Destructure ingredientInfo
        addIngredient(recipe, name, amount, unit);

        // 4. Update the recipe in storage
        // Create a success message first
  ...
  .command('quick [time]', 'Find recipes that can be made quickly', (yargs) => {
    ...
    // Use the getQuickRecipesList helper function
    getQuickRecipesList(argv.time, function (quickRecipes, maxTime) {
      if (quickRecipes.length === 0) {
        console.log(`No recipes found under ${maxTime} minutes.`);
      } else {
        console.log(`Recipes that can be made in ${maxTime} minutes or less:`);
        quickRecipes.forEach(recipe => {
          console.log(`- ${recipe.name} (Cooking Time: ${recipe.cookingTime} minutes)`);
        });
      }
    });
  })
